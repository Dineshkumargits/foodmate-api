...
// Daily reminder for sellers to add food entries - 9 AM
...
// Month-start payment reminders - First 3 days of month at 10 AM
export const scheduleMonthEndPaymentReminders = () => {
  // Run every day at 10 AM, but only send on first 3 days of month
  cron.schedule("0 10 * * *", async () => {
    ...
    console.log("Running month-start payment reminders for previous month...");
    try {
      const sellers = await User.findAll({
        where: { role: "seller" },
      });
...
export const initializeSchedulers = () => {
  ...
  console.log("- Daily food entry reminders for sellers: 9:00 AM");
  console.log(
    "- Month-start payment reminders: 10:00 AM (first 3 days of month)"
  );
};
